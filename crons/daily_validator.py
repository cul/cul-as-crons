...
class DailyValidator(object):
    ...
    def run(self):
        # TODO: we'll probably want to log changes somehow, so if a record hasn't been updated manually, we're not going through all the children again
        """Run validation across a repository for updated (or new) resource records."""
        # new_resources = self.as_client.get_new_resources(2, 1669672800)
        new_resources = self.as_client.get_new_resources(2, yesterday_utc())
        for resource in new_resources:
            ResourceValidator().run(resource)
...
class ResourceValidator(object):
    ...
    def run(self, resource):
        ...
        Args:
            resource (obj): ArchivesSnake JSONModel object for a resource record
        """
    ...
    def published_in_clio(self):
        """Checks if a collection is in CLIO."""
        try:
    ...
    def dacs_compliance(self):
        """Checks resource for DACS compliance."""
    ...
    def check_published_agents(self):
        ...
        if not unpublished_agents:
            for ua in unpublished_agents:
                ...
                if match:
                    logging.info(f"Unpublished agent found: {ua.ref}")
